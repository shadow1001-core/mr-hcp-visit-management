...
1. 锁定拜访行；
2. 首次提交验证状态为 `CHECKED_OUT`；整体更新验证状态为 `REPORTED`；
3. 校验沟通产品为计划产品的非空、不重复子集；
4. 创建报告，或显式删除旧子项后整体重建当前报告；
5. 首次提交后状态派生为 `REPORTED`；更新时保持 `REPORTED`；
6. 提交事务。

报告及其全部子项必须同时成功或同时回滚。报告更新不写入 `visits` 或
`compliance_findings`，因此不会改变现场事实和合规结论。
